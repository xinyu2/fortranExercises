...
!-- x-boundary
  ...
  do jj=1,gas_ny
     ...
     !write(6,*) 'ghostx',impi,jj,gas_ighostx(jj)
  enddo
!-- y-boundary
  ixoffset = gas_nx*(impi)
  iyoffset = gas_ny
  ...
  do ii=1,gas_nx
     i = ii+ixoffset
     j = iyoffset
     gas_ighosty(ii) = -1 !grd_icell(i,j)
  enddo
...
!-- TRANSPORT:
  ...
  prt_ndeadall = 0
  itotiter = 0
  do while (.not. prt_lalldone .and. itotiter<100)
     !if (lmpi0) then
...
!-- repost the nonblocking receive
     ...
     call mpi_irecv(prt_rcvbuffer(:,1),nbuffsize,MPI_BYTE, &
          gas_neighbors(2),1,MPI_COMM_WORLD,irequest(1),ierr)
...
!-- repost the nonblocking receive
     ...
     call mpi_irecv(prt_rcvbuffer(:,2),nbuffsize,MPI_BYTE, &
          gas_neighbors(1),2,MPI_COMM_WORLD,irequest(2),ierr)
...
!-- send the buffer to the x-neighbor
                       write(6,*) 'sned',itotiter,impi,gas_neighbors(1)
...
!-- send the local particle completed count
           ...
           call mpi_send(prt_ndead,1,MPI_INTEGER,impi0,3, &
                MPI_COMM_WORLD,ierr)
        endif
