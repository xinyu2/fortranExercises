...
c-- copy
      ...
      ny = ndim(2)
      nz = ndim(3)
!     milargro dd: check nmpi is good for decompose
!     for igeom=2, each rank expect perfect square cells
!     for igeom=3, each rank expect perfect cube cells
      select case(igeom)
      case(1)
      case(2)
         remainder=mod(sqrt((nx*ny*nz)/(nmpi+0d0)),2d0)
         if(remainder==0) then
