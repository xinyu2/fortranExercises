     ...
     if (.not.existLocalPar) then
        ...
        if(impi==0) then
           ...
           if(pcmplt==maxpars) then
              globalFinish=.true.
              do i=1,nmpi-1
                 call mpi_send(globalFinish,1,MPI_LOGICAL,i,&
...
contains
  ...
  subroutine initRank(myrank,ps,sndbuff,sndidx,rcvbuff)
    implicit none
    integer, intent(in)::myrank
...
!-- add incoming particles to main particle array on this rank
             do rcvidx = 1, BUFFSIZE ! maxbfsize(i)
                ...
                rcvbuff(i,rcvidx)%r=-1
                rcvbuff(i,rcvidx)%e=0
                rcvbuff(i,rcvidx)%pid=0
